...
@Component({
  selector: 'app-offer-land-pange',
  templateUrl: './offer-land-pange.page.html',
  styleUrls: ['./offer-land-pange.page.scss'],
})
...
ionViewWillEnter()
{
  ...
  if (document.URL.indexOf("/") > 0) {
   ...
   this.sellerName= this.platform.getQueryParam("seller");
   this.influencerId= this.platform.getQueryParam("influencer");
...
{
  this.dbService.getOfferVendorProducts(this.offerId, data_vendor._id).toPromise()
  .then((data:any)=>
{
  console.log(data)
  if(data.ok)
